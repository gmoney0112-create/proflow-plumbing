...
export const CallToAction: React.FC<{ localFrame: number }> = ({ localFrame }) => {
  ...
  return (
    <div style={{ width, height, background: "radial-gradient(ellipse at 50% 50%, #0d1f3c 0%, #060d1a 70%)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", opacity: bgOpacity, position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "absolute", inset: 0, background: "radial-gradient(ellipse at 50% 50%, rgba(13,31,60,0.82) 0%, rgba(6,13,26,0.9) 70%)" }} />
      ...
      <div style={{ background: "#ff4444", color: "#fff", fontSize: 14, fontFamily: "Arial, sans-serif", fontWeight: 700, letterSpacing: 4, textTransform: "uppercase", padding: "8px 24px", borderRadius: 4, marginBottom: 30, opacity: interpolate(localFrame, [5, 20], [0, 1], { extrapolateRight: "clamp" }), boxShadow: "0 0 20px rgba(255,68,68,0.5)", position: "relative", zIndex: 2 }}>● AVAILABLE RIGHT NOW</div>
      <div style={{ textAlign: "center", marginBottom: 20, position: "relative", zIndex: 2 }}><AnimatedText text="Got a plumbing problem?" delay={10} animationType="slide-up" style={{ fontSize: 40, fontFamily: "Arial, sans-serif", color: "#8ecfff", fontWeight: 400 }} /></div>
      <div style={{ textAlign: "center", marginBottom: 50, position: "relative", zIndex: 2 }}><AnimatedText text="We Fix It Fast." delay={25} animationType="scale-in" style={{ fontSize: 90, fontWeight: 900, fontFamily: "'Arial Black', sans-serif", color: "#ffffff", lineHeight: 1, textShadow: "0 0 40px rgba(0,149,255,0.4)", display: "inline-block" }} /></div>
      ...
      <div style={{ transform: `scale(${btnScale})`, background: "linear-gradient(135deg, #0095ff, #00d4ff)", borderRadius: 50, padding: "22px 70px", fontSize: 26, fontWeight: 800, fontFamily: "Arial, sans-serif", color: "#fff", letterSpacing: 2, textTransform: "uppercase", boxShadow: "0 0 40px rgba(0,149,255,0.6), 0 8px 32px rgba(0,0,0,0.4)", position: "relative", zIndex: 2 }}>Book a Free Estimate →</div>
